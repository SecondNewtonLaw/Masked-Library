...
public partial class MemoryUnsafe {
    ...
    public static void PinManagedObject<T>(T obj) where T : notnull {
        ...
        MemoryKeeper.pointer_keeper.AddOrUpdate(key, keyI => keyI == key ? handle : GCHandle.FromIntPtr(nint.Zero),
            (_, val) => val);
    }
    ...
    public static void UnpinManagedObject<T>(T obj) where T : notnull {
        ...
        if (!MemoryKeeper.pointer_keeper.Remove(target, out var handle))
    ...
    public static unsafe void* GetPointerToManagedObject<T>(ref T obj) where T : notnull {
    ...
    public static unsafe void GetManagedObjectFromPointer<T>(void* ptr, out T obj) where T : notnull {
